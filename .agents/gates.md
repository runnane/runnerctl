...
`make gates` = `lint` + `example-drift` + `smoke` + `sim`. All four run without
...
adding `disable` directives; the one existing directive (SC1090, sourcing a
path only known at runtime) is inherent to the config-file design.
